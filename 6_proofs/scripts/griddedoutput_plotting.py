...
def _timeLabel(DataStruct, t_idx, ax, **kwargs):
    time = int((t_idx - 1)*DataStruct.historydelta_m/60)
...
def plotNumberDist(DataStruct, number_dist_arr, plot_type='hist', **kwargs):
    ...
    if plot_type == 'line':
        ...
        if kwargs.get('t_idx'):
            t = kwargs.get('t_idx')
            history_dt = DataStruct.historydelta_m/60 # hours
            time =  ((t-1)*history_dt)
            label = f't = {time:3.2f} hrs'

        lw = kwargs.get('lw', 1)
        ls = kwargs.get('ls', '-')
        ax.plot(DataStruct.bin_geocenter, number_dist_arr, c=c, label=label, lw=lw, ls=ls)
    elif plot_type == 'hist':
        bin_width = DataStruct.bin_edges[1:] - DataStruct.bin_edges[:-1] 
        ax.bar(x=DataStruct.bin_geocenter, height=number_dist_arr, width=bin_width, alpha=.6)
    if kwargs.get('ylims'):
...
def plotSpeciatedMassDist(DataStruct, binned_species_mass_arr, **kwargs):

    if binned_species_mass_arr.shape[1] == DataStruct.n_bins:
        binned_species_mass_arr = binned_species_mass_arr[:, :-1]

    bin_total_mass = np.zeros(((DataStruct.n_bins-1),))
    fig, ax = plt.subplots(1, 1)
    plt.rcParams['axes.prop_cycle'] = cycler('color', plt.get_cmap('tab20').colors)

    # Plot the total speciated mass distribution
    for i, species in enumerate(DataStruct.aero_species):
    ...
    #ax.plot(bin_edges[:-1], bin_total_mass, label=species)
        if i == 0:
            ...
            y_lower = np.array((DataStruct.n_bins-1)*[y_lower])
        else:
            y_lower = bin_total_mass - species_mass_dist
        ax.fill_between(x=DataStruct.bin_geocenter, y1=y_lower, y2=bin_total_mass, label=species)
...
def plotSpeciatedMassFrac(DataStruct, binned_species_mass_arr, **kwargs):
    ...
    for i, species in enumerate(DataStruct.aero_species):
        ...
        ax.fill_between(x=DataStruct.bin_geocenter, y1=y_lower, y2=frac_total, label=species)

    plt.xlim(DataStruct.bin_edges[0], DataStruct.bin_edges[-1])
    plt.ylim(0, 1)
    plt.xscale('log')
...
def construct_figure_filename(DataStruct, figure_type, **kwargs):
...
def plotFourPanelMassFrac(DataStruct, scenario, times, xstart, xend, ystart, yend, z_idx, **kwargs):
    ...
    for j, (ax, t) in enumerate(zip(axs.flatten(), times)):
        ...
        DataStruct.loadData(scenario, xstart, xend, ystart, yend, z_idx, t)
        binned_species_mass_arr =  getBinnedSpeciesMassOptimized(DataStruct.gridded_data[scenario]['aero_diams'],
                                                                DataStruct.gridded_data[scenario]['aero_masses'], 
                                                                DataStruct.gridded_data[scenario]['aero_numconc'], 
                                                                n_grid_cells=DataStruct.gridded_data[scenario]['n_total_cells'])
        ...
        # Plot the total speciated mass distribution
        aero_species = [var for var in DataStruct.aero_vars if 'pmc_' in var]
        for i, species in enumerate(aero_species):
            ...
            variable_fmt = DataStruct.aerosol_fmt_map[species]
            ...
            ax.fill_between(x=DataStruct.bin_geocenter, y1=y_lower, y2=frac_total, label=variable_fmt)

        # set xlim, ylim, axis scaling
        ax.set_xlim(DataStruct.bin_edges[0], DataStruct.bin_edges[-1])
        ax.set_ylim(0, 1)
        ax.set_xscale('log')
        if kwargs.get('xlims'):
            ax.set_xlim(kwargs.get('xlims'))
...
def plotScenarios2DKappaDists(DataStruct, scenarios, xstart, xend, ystart, yend, z_idx, t_idx):
    ...
    for i, (ax, scenario) in enumerate(zip(axs.flatten(), scenarios)):
        DataStruct.loadData(scenario, xstart, xend, ystart, yend, z_idx, t_idx)

        # 1-d arrays with per-particle attributes
        kappa = DataStruct.gridded_data[scenario]['aero_kappa']
        diams = DataStruct.gridded_data[scenario]['aero_diams']
        numconc = DataStruct.gridded_data[scenario]['aero_numconc']
        n_cells = DataStruct.gridded_data[scenario]['n_total_cells']
...
def plotScenario2DKappaDist4Panel(DataStruct, scenario, xstart, xend, ystart, yend, z_idx):
    # plot from kappa=(0,1) and diameters=(1e-8, 1e-6), weight by the number conc. of each computation particle. 
...
def plotScenarios1DKappaDists(DataStruct, scenarios, xstart, xend, ystart, yend, z_idx, t_idx):
    general_labels = {'uniform-basecase': 'Uniform base case', 
                'point-source-1x1': 'Scenario 3'}
