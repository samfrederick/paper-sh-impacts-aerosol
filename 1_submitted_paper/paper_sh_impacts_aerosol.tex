...
\begin{figure}[!h]
	...
	\caption{Time-height plots of temperature (left) and relative humidity (right) showing horizontal
     cross section averages of each variable at 10-minute intervals.}
	\label{fig:height-time-temp-rh}
\end{figure} 

Gas-aerosol partitioning of semi-volatile compounds is determined by
both temperature and relative humidity. Time-height plots of each
environmental variable are shown in Figure
\ref{fig:height-time-temp-rh}, displaying the average values across
each vertical level. Near-surface temperature increases from 300~K to
307~K due to constant surface heating, while the the upper part of the
growing boundary layer warms from initially 291~K to 294~K. As
the convective boundary layer deepens and warms, surface
relative humidity decreases from initially 44$\%$ to 28$\%$ after
6~hours. RH aloft also decreases, from 69$\%$ to 48$\%$. 
