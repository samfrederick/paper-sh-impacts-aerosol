...
Key findings of this paper are structured as follows. \textcolor{red}{Section \ref{sec:environment}
discusses environmental conditions throughout simulations.} 
Section \ref{sec:gas-impacts} investigates the relationship between emissions
...
\begin{figure}[!h]
	\centering
	\includegraphics[]{figures/height-time-temp-rh.pdf}
	\caption{\textcolor{red}{Time-height plots of temperature (left) and relative humidity (right). Plots display the horizontal
     cross section average of each variable at regular 10 minute intervals.}}
	\label{fig:height-time-temp-rh}
\end{figure} 

\textcolor{red}{Equilibirum partitioning of semi-volatile compounds between the gas and condensed aerosol phases
is determined by both temperature and relative humidity. Time-height plots of each environmental variable are shown in 
Figure \ref{fig:height-time-temp-rh}, displaying the average value across each vertical level and time. Temperature is
warmest near the surface, increasing from 300~K to 306.8~K over the duration of the simulation due to constant surface 
heating. Temperature in the upper boundary layer increases from initially 291~K to 294~K. As the convective boundary layer
grows and continues to warm, surface relative humidity decreases from initially 44$\%$ to 28$\%$ after 6~hours. Drying out 
of the upper boundary layer is also observed, with relative humidity decreasing from 69$\%$ to 47.7$\%$.}
...
reduction in OH near the emissions plume. Whereas in the no heterogeneity scenario,
the level-mean mixing ratio of OH is 0.61~pptv at $z=1$~km,
whereas in the high heterogeneity scenario they drop by 54\%
to 0.28~pptv.
...
nitrate for the no heterogeneity scenario appears higher ($\eta=0.513$) than
...
sulfate. By contrast, particles in the high heterogeneity scenario are dominated by
...
\ref{fig:speciated-mass-frac} indicates that, for the no heterogeneity scenario, 
particles in this size range are primarily composed of low
hygroscopicity compounds, including BC and POA. In the high heterogeneity scenario, the
...
particles), whereas $\kappa$ only reaches up to 0.4 in the no heterogeneity scenario. 
...
decrease, particularly at $S_{\mathrm{env}}=1.0\%$, where CCN concentrations drop
below all other scenarios including the no heterogeneity scenario. This is due to 
...
At higher supersaturations ($S_{\mathrm{env}}=1.0\%$), the high heterogeneity scenario exhibits lower CCN
concentrations than the ammonia-free no heterogeneity scenario. This further confirms that, 
in the absence of
...
cases of the high heterogeneity scenario and the no heterogeneity scenario, apart from a slight
(~5\%) downward shift in the high heterogeneity scenario due to enhanced coagulation,
