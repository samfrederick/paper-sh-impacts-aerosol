...
%DIF DEL 1_submitted_paper/paper_sh_impacts_aerosol.tex   Fri Sep 26 10:11:21 2025
%DIF ADD 4_revised_paper/paper_sh_impacts_aerosol.tex     Sat Feb 14 14:31:54 2026
%% ---------------------------------
...
spatially heterogeneous emissions \citep{qian_investigation_2010}, yet \DIFdelbegin \DIFdel{many climate models fail to resolve this variability adequately. }\DIFdelend \DIFaddbegin \DIFadd{most climate models do not explicitly represent this variability in a general sense. Existing parameterizations in global climate models for sub-grid scale emissions and associated processes are typically limited to specific phenomena such as ship tracks \mbox{%DIFAUXCMD
\citep{huszar_modeling_2010} }\hskip0pt%DIFAUXCMD
and contrails \mbox{%DIFAUXCMD
\citep{burkhardt_processbased_2009}}\hskip0pt%DIFAUXCMD
, rather than providing a general treatment of emissions-driven aerosol heterogeneity.
}\DIFaddend 
...
employs a \DIFdelbegin \DIFdel{10-bin sectional scheme }\DIFdelend \DIFaddbegin \DIFadd{17-bin sectional scheme to represent externally mixed
aerosol populations with differing hygroscopicities}\DIFaddend , while DALES
...
multiphase chemistry. It \DIFdelbegin \DIFdel{is }\DIFdelend extends the aerosol-transport column model
...
regional domain driven by \DIFdelbegin \DIFdel{by }\DIFdelend simulated meteorology
...
using a constant rate of \DIFdelbegin \DIFdel{0.24~$\rm K \, m \, s^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{295.5~$\rm W \, m^{-2}$}\DIFaddend , which is
representative of \DIFdelbegin \DIFdel{typical kinematic heat fluxes observed during
mid-day in clear sky conditions over land
\mbox{%DIFAUXCMD
\citep{stull_introduction_1988}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{surface heat fluxes resulting from annually-averaged mid-latitude solar insolation}\DIFaddend .
...
(POA) by mass.  \DIFdelbegin \DIFdel{The three emission }\DIFdelend \DIFaddbegin \DIFadd{Aerosol emissions are comprised of three }\DIFaddend modes representing cooking and vehicular combustion\DIFdelbegin \DIFdel{comprise varying mixtures of POA and BC. The
three emission modes, representing cooking and vehicular combustion
sources, each consist }\DIFdelend \DIFaddbegin \DIFadd{. Each emission mode consists }\DIFaddend of distinct fixed mixtures of \DIFdelbegin \DIFdel{primary organic
aerosol (POA ) }\DIFdelend \DIFaddbegin \DIFadd{POA }\DIFaddend and black carbon (BC). To allow for simulation spin-up
...
\citep{mohebalhojeh_2024}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{mohebalhojeh_metric_2026}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The metric $\eta$ is a normalized
...
maximum possible value of this average difference given a fixed domain mean. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{mohebalhojeh_2024} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{mohebalhojeh_metric_2026} }\hskip0pt%DIFAUXCMD
}\DIFaddend show that the metric is
...
307~K due to constant surface heating, while the \DIFdelbegin \DIFdel{the }\DIFdelend upper part of the
...
Conversely, when the gas phase concentration \DIFaddbegin \DIFadd{of }\DIFaddend nitric acid is high such as near an 
...
acid mixing ratios decrease on average. As will \DIFaddbegin \DIFadd{be }\DIFaddend demonstrated in
Section \ref{sec:aero-comp}, this is due to increased partitioning of
these species into the aerosol phase to form ammonium
nitrate. \DIFaddbegin \DIFadd{Comparing modeled ammonium and nitric acid levels with past
...
California \mbox{%DIFAUXCMD
\citep{salmon_nitric_1990, toro_sensitivity_2024}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
with high emissions spatial heterogeneity result \DIFaddbegin \DIFadd{in }\DIFaddend a pronounced
reduction in OH near the emissions plume. \DIFdelbegin \DIFdel{Whereas in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend the no heterogeneity scenario,
the level-mean mixing ratio of OH is \DIFdelbegin \DIFdel{0.61}\DIFdelend \DIFaddbegin \DIFadd{0.64}\DIFaddend ~pptv at $z=1$~km\DIFdelbegin \DIFdel{,
whereas in the }\DIFdelend \DIFaddbegin \DIFadd{. Near the emissions plume of the }\DIFaddend high heterogeneity scenario\DIFdelbegin \DIFdel{they drop by 54\% to 0.28}\DIFdelend \DIFaddbegin \DIFadd{, the concentration of OH drops by 58\% to 0.27}\DIFaddend ~pptv.
...
particles decreases by up to \DIFdelbegin \DIFdel{81}\DIFdelend \DIFaddbegin \DIFadd{63}\DIFaddend \% while the number of accumulation
mode particles increases by up to \DIFdelbegin \DIFdel{246}\DIFdelend \DIFaddbegin \DIFadd{232}\DIFaddend \%. The decrease in Aitken mode
...
up to \DIFdelbegin \DIFdel{74}\DIFdelend \DIFaddbegin \DIFadd{49}\DIFaddend \% for high emissions spatial heterogeneity scenarios, while
the accumulation mode mass fraction increases by up to
\DIFdelbegin \DIFdel{309}\DIFdelend \DIFaddbegin \DIFadd{238}\DIFaddend \%. Coagulation of smaller Aitken mode particles with accumulation
...
\begin{figure}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[]{figures/aerosol-SNA-spatial-heterogeneity-time36-z45.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[]{figures/aero-spatial-heterogeneity-time36-z45.pdf}
	\DIFaddendFL \caption{Cross sections in the x-y plane of aerosol species
...
(\DIFdelbegin \DIFdel{$\eta=0.021$}\DIFdelend \DIFaddbegin \DIFadd{$\eta=0.020$}\DIFaddend ) compared to ammonium (\DIFdelbegin \DIFdel{$\eta = 0.142$}\DIFdelend \DIFaddbegin \DIFadd{$\eta = 0.153$}\DIFaddend ) and nitrate
(\DIFdelbegin \DIFdel{$\eta=0.739$}\DIFdelend \DIFaddbegin \DIFadd{$\eta=0.694$}\DIFaddend ).  This is due to the extremely low volatility of
...
nitrate for the no heterogeneity scenario appears higher (\DIFdelbegin \DIFdel{$\eta=0.513$}\DIFdelend \DIFaddbegin \DIFadd{$\eta=0.446$}\DIFaddend ) than
the value of $\eta$ for ammonium \DIFaddbegin \DIFadd{($\eta=0.012$) }\DIFaddend and sulfate (\DIFdelbegin \DIFdel{$\eta=0.010$ and
$\eta=0.011$, respectively}\DIFdelend \DIFaddbegin \DIFadd{$\eta=0.012$}\DIFaddend ).
...
(VOCs) \DIFdelbegin \DIFdel{complete }\DIFdelend \DIFaddbegin \DIFadd{compete }\DIFaddend alongside SO$_2$ for oxidation. With OH rapidly
...
\begin{figure}[!h]
	...
	\caption{Speciated mass fraction as a function of particle
          ...
          after 6 hours. \DIFaddbeginFL \DIFaddFL{Hatching indicates particles in the size range $D_{\mathrm{p}}\sim50$--$100$~nm, for which CCN activity highly depends on composition.}\DIFaddendFL }
...
sulfate \DIFaddbegin \DIFadd{and ammonium}\DIFaddend . By contrast, particles in the high heterogeneity scenario are dominated by
nitrate, ammonium, and sulfate\DIFdelbegin \DIFdel{, which together comprise 50--80\% of
aerosol mass}\DIFdelend .

The CCN activity of particles in the size range of 50--100\DIFdelbegin \DIFdel{nm }\DIFdelend \DIFaddbegin \DIFadd{~nm (hatched region in Figure \ref{fig:speciated-mass-frac}) }\DIFaddend is
largely governed by their composition. \DIFdelbegin \DIFdel{Figure
\ref{fig:speciated-mass-frac} indicates that, for }\DIFdelend \DIFaddbegin \DIFadd{For }\DIFaddend the no heterogeneity scenario, 
particles in this size range are \DIFdelbegin \DIFdel{primarily composed of }\DIFdelend \DIFaddbegin \DIFadd{composed of 50--70\% }\DIFaddend low
hygroscopicity compounds \DIFaddbegin \DIFadd{by mass}\DIFaddend , including BC\DIFdelbegin \DIFdel{and POA}\DIFdelend \DIFaddbegin \DIFadd{, POA, and SOA}\DIFaddend . In the high heterogeneity scenario, \DIFdelbegin \DIFdel{the
dominant presence of }\DIFdelend \DIFaddbegin \DIFadd{particles in the range of 50--100~nm are mostly formed of highly hygroscopic species, with }\DIFaddend sulfate, nitrate, and ammonium \DIFdelbegin \DIFdel{increases particle hygroscopicity. 
}\DIFdelend \DIFaddbegin \DIFadd{comprising 60--80\% of particle mass. 
}

\DIFaddend This suggests that particles in the size range of
...
particles), whereas \DIFdelbegin \DIFdel{$\kappa$ only reaches up to 0.4 }\DIFdelend \DIFaddbegin \DIFadd{particles of this size only reach up to $\kappa\sim0.4$ }\DIFaddend in the no heterogeneity scenario. 
...
concentrations, explaining the \DIFdelbegin \DIFdel{absence of a }\DIFdelend \DIFaddbegin \DIFadd{reduction in }\DIFaddend low-$\kappa$ carbonaceous
...
decrease, particularly at $S_{\mathrm{env}}=1.0\%$, where CCN concentrations \DIFdelbegin \DIFdel{drop
below all other scenarios including }\DIFdelend \DIFaddbegin \DIFadd{at most levels are comparable with }\DIFaddend the no heterogeneity scenario. This is due to 
...
\begin{figure}[!h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[]{figures/aerosol-ccn-vertical-profiles-no-nh4-cases-time36.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[]{figures/ammonia-free-vertical-profiles-time36.pdf}
	\DIFaddendFL \caption{Vertical profiles for CCN concentrations in
...
At higher supersaturations ($S_{\mathrm{env}}=1.0\%$), the high heterogeneity\DIFaddbegin \DIFadd{, ammonia-free }\DIFaddend scenario exhibits lower CCN
concentrations than the \DIFdelbegin \DIFdel{ammonia-free no heterogeneity scenario}\DIFdelend \DIFaddbegin \DIFadd{corresponding no-heterogeneity condition}\DIFaddend . This further confirms that, 
in the absence of
...
CCN. \DIFdelbegin \DIFdel{The similarity in CCN profiles between the ammonia-free
cases of the }\DIFdelend \DIFaddbegin \DIFadd{These findings underscore that the impact of emissions spatial heterogeneity on CCN activity is highly modulated by atmospheric chemical composition and the competing influence of gas-particle partitioning and coagulation.
}


\subsection{\DIFadd{Impacts of reversible partitioning on aerosol aging}}\label{sec:reversible-aging}
\DIFadd{Up to this point, simulations discussed in this paper all follow a similar timeline for emissions, with gases and primary aerosol emitted starting at $t=1$~h and continuing through the remainder of each simulation at a constant rate. Here, we explore how aerosol composition changes if emissions are turned off part way through the simulation. Such a setup is akin to a Lagrangian framework of advecting a column over an emissions source and away from the source after a period of time. This allows investigating the timescale of reversible partitioning for semivolatile compounds such as nitrate and the ``memory'' of the aerosol state to emissions heterogeneity. 
}

\DIFadd{We ran an additional simulation for the }\DIFaddend high heterogeneity scenario \DIFaddbegin \DIFadd{in which emissions were turned off at $t=4$~h. Figure \ref{fig:nh4-no3-relaxation} shows vertical profiles of ammonium and nitrate at 1-hour intervals for the high heterogeneity scenario in which emissions are turned off at $t=4$~h (left column), alongside the corresponding scenario in which  emissions continued through the end of the simulation at $t=6$~h (right column).
}

\DIFadd{Once emissions are turned off, meaningful differences emerge between the aerosol composition in each scenario. The impact of turning off emissions is most apparent for nitrate, which decreases from a peak concentration of $0.9$~ppbv to $0.27$~ppbv between $t=4$~h and $t=6$~h. Ammonium levels also decrease through $t=6$~h, however most remains in the condensed phase. This indicates that as nitrate levels decrease, some free ammonia re-enters the gas phase, however most remains in the aerosol as ammonium sulfate. These findings indicate that the timescale for complete reversal of the impact of emissions heterogeneity on the aerosol state depends on the reversible partitioning of nitrate. The rate of this repartitioning, in turn, depends on the strength of mixing and dilution within the boundary layer. As a result, aerosol populations retain a finite “memory” of prior exposure to heterogeneous emissions, even after the source influence has ceased, an effect that is largely lost in coarse-resolution models that rely on grid-mean aerosol properties.
}\begin{figure}[!h]
	\centering
	\includegraphics[]{figures/high-het-relaxation.pdf}
	\caption{\DIFaddFL{Vertical profiles of ammonium (top row) and nitrate (bottom row) for two versions of the high heterogeneity scenario in which emissions are turned off at $t=4$~h (left column) and emissions continue through the remainder of the simulation (right column).}}
	\label{fig:nh4-no3-relaxation}
\end{figure}

\subsection{\DIFadd{Modulation of aerosol composition and CCN activity by co-condensation}}\label{sec:co-condensation}
\DIFadd{Gas–particle partitioning of semivolatile species is strongly modulated by ambient relative humidity, as water uptake can promote co-condensation well below water saturation, altering particle size and composition prior to activation \mbox{%DIFAUXCMD
\citep{topping_tight_2012}}\hskip0pt%DIFAUXCMD
. 
...
introduces limitations \DIFdelbegin \DIFdel{. First, the simulations }\DIFdelend \DIFaddbegin \DIFadd{and opportunities for future investigation.
}

\DIFadd{Simulations in this study }\DIFaddend assume a horizontally homogeneous land
...
shear, and diurnal variability. \DIFdelbegin \DIFdel{Second}\DIFdelend \DIFaddbegin \DIFadd{Additionally}\DIFaddend , all emissions—both
gas-phase and particulate—are temporally constant after spin-up and
spatially collocated, \DIFdelbegin \DIFdel{which may not reflect }\DIFdelend \DIFaddbegin \DIFadd{reflecting a deliberate idealization used here
to isolate the effects of emissions spatial heterogeneity.  This
configuration may not fully capture }\DIFaddend the complexity of real urban
emission patterns where different sources (e.g., traffic, industry,
biomass burning) are spatially and temporally decoupled. \DIFdelbegin \DIFdel{Third, }\DIFdelend \DIFaddbegin \DIFadd{Future
...
species. Emission flux observations and inventories are not uniformly available at 100 m resolution. While many gridded inventories used in regional and global-scale modeling are too coarse for direct use in LES, several important source types can be represented accurately at sub-kilometer scales. Point sources (e.g., power plants, industrial stacks) and line sources (e.g., road networks) are often well constrained spatially and can be implemented directly in high-resolution simulations. In contrast, diffuse area sources that are typically reported at county or regional scales require additional emission preprocessing, downscaling, or data-fusion approaches to distribute fluxes at finer resolution. Developing such preprocessing workflows represents an important step toward applying WRF-PartMC-LES in observationally constrained, realistic settings.
}



\DIFadd{Furthermore, }\DIFaddend while the chosen aerosol composition is
...
generalizability of the results. \DIFdelbegin \DIFdel{Finally, }\DIFdelend \DIFaddbegin \DIFadd{We also wish to acknowledge that }\DIFaddend the computational particle
...
\DIFadd{Lastly, nucleation is not modeled in WRF-PartMC-LES due to the large computational expense associated with explicitly representing the coagulation of many ultrafine computational particles. As a result, ultrafine particle concentrations may be
...
\DIFaddend These limitations motivate future work incorporating more realistic
emissions, meteorological variability, \DIFdelbegin \DIFdel{and }\DIFdelend extended simulation times\DIFaddbegin \DIFadd{, and improved representation of physical processes }\DIFaddend to evaluate the robustness of the conclusions presented here.
...
\DIFdelbegin %DIFDELCMD < \codedataavailability{WRF-PartMC is open source and maintained on GitHub: https://github.com/open-atmos/wrf-partmc.git. The version of
%DIFDELCMD < WRF-PartMC used in this paper was coupled with the multiphase chemistry model MOSAIC and custom initialization code was written for 
%DIFDELCMD < specifying the emissions initial conditions unique to each emissions scenario alongside specific namelist configurations for particle-resolved 
%DIFDELCMD < LES. The model code (including the WRF base model v3.9.1, PartMC aerosol model v2.6.1, and MOSAIC chemistry model) 
%DIFDELCMD < used for this publication has been archived on Zenodo with the following DOI: https://doi.org/10.5281/zenodo.16850289.
%DIFDELCMD < All simulation outputs and scripts used to generate figures in this paper are available via the Illinois Data Bank: https://doi.org/10.13012/B2IDB-9622921\_V1.} %%%
\DIFdelend \DIFaddbegin \codedataavailability{WRF-PartMC is open source and maintained on GitHub: https://github.com/open-atmos/wrf-partmc.git. The version of
...
used for this publication has been archived on Zenodo with the following DOI: https://doi.org/10.5281/zenodo.18644307.
All simulation outputs and scripts used to generate figures in this paper are available via the Illinois Data Bank: https://doi.org/10.13012/B2IDB-9622921\_V3.} \DIFaddend %% use this section when having data sets and software code available
