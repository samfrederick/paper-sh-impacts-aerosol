...
%DIF ADD ./paper_sh_impacts_aerosol.tex                      Fri Feb  6 17:03:41 2026
%% ---------------------------------
...
spatially heterogeneous emissions \citep{qian_investigation_2010}, yet many climate models fail to resolve this variability adequately\DIFaddbegin \DIFadd{; existing parameterizations in GCMs for sub-grid scale emissions and associated processes are limited to specific phenomena such as ship tracks \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{10-bin sectional scheme }\DIFdelend \DIFaddbegin \DIFadd{17-bin sectional scheme to represent both internally and externally mixed particles}\DIFaddend , while DALES modifies the seven-mode M7 model
...
these species into the aerosol phase to form ammonium nitrate. \DIFaddbegin \DIFadd{Comparing modeled ammonium and nitric acid levels with past studies, concentrations of each are consistent with brief, extremely polluted conditions found in regions with a large volume of vehicular emissions, such as Southern California \mbox{%DIFAUXCMD
...
\DIFadd{Simulations in this study }\DIFaddend assume a horizontally
...
surface heterogeneity, wind shear, and diurnal variability. \DIFdelbegin \DIFdel{Second}\DIFdelend \DIFaddbegin \DIFadd{Additionally}\DIFaddend ,
...
temporally decoupled. \DIFdelbegin \DIFdel{Third}\DIFdelend \DIFaddbegin \DIFadd{Future studies should investigate the impact of emissions heterogeneity on the aerosol state in response to realistic emission patterns such as numerous point sources with spatially segregated reactive species. Gridded inventories commonly used for regional and global-scale modeling are too coarse for use with LES. Future extensions of WRF-PartMC-LES to realistic emission patterns will require use of data fusion techniques to estimate source apportionment at sub-kilometer resolution.
}
...
\DIFadd{Our study of the effects of co-condensation reveals meaningful limitations in the manner by which WRF-PartMC-LES estimates CCN activity for sub-saturated particles. While our study reveals that emissions heterogeneity may alter aerosol composition and CCN activity even when co-condensation is considered, further work is required to place quantitative bounds on the contribution of each process and the modulating role of gas and aerosol composition. Furthermore, we wish to emphasize that we do not model explicitly model CCN activation and droplet growth. Although previous particle-resolved studies have included this effect \mbox{%DIFAUXCMD
...
\DIFadd{In this study, use of LES is primarily motivated by the length scale of heterogeneous emission patterns. The current work resolves emissions heterogeneity down to 100 m, significantly below regional and global-scale model resolutions. 
