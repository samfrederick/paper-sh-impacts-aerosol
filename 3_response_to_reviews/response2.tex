...
diﬀerent outcome. Perhaps, since this study is highly idealized,
...
\begin{quote}
  ...
  We have included the following statement in Section 4, lines 447--454 acknowledging our limited treatment of CCN activity and anticipated outcomes if explicit modeling of CCN activation at subsaturated conditions were incorporated:
...
\textbf{(2.2)} Could you use a proﬁle where there’s a more humid boundary layer? I think a
...
multiple point sources (plumes) and testing sensitivity to that conﬁguration vs.
...
\begin{quote}
  We agree that more realistic emission patterns should be explored, and have included the following statement in Section 4, lines 424--430:
...
turbulence (only brieﬂy mentioned in the introduction) which I would assume is
...
\begin{quote}
We have included the following statement in Section 4, lines 455--464
...
\textbf{(2.5)} Following from question 4 above, emission ﬂux data observations are not at 100
m resolution, correct? If so, how feasible is this framework for testing against
observations for future implementation?

\begin{quote}
  We have added the following discussion on lines 430--436 to clarify
...
\textbf{(2.6)} For some of the ﬁgures, for example Figure 7, it would be nice to give the
